...
(function () {
    ...
    lazyLoad.provider("$lazyLoad", ['$controllerProvider', '$compileProvider', '$filterProvider', '$provide', function ($controllerProvider, $compileProvider, $filterProvider, $provide) {
        ...
        this.$get = ['$document', '$injector', '$interval', 'cacheMod', '$http', '$compile', '$templateCache', function ($document, $injector, $interval, cacheMod, $http, $compile, $templateCache) {
            return {
                ...
                loadModule: function (options, afterLoad) {
                    ...
                    if (!options.scope) {
                        throw ("Не передан scope к которому привязывать модуль!");
                    } else {
                        var scope = options.scope;
                    }
                    //Элемент в который нужно вставить подгруженный шаблон
                    if (!options.el) {
                        //console.error("Вы не указали елемент в который нужно подгрузить шаблон!");
                        var elm = null;
                    } else {
                        var elm = options.el;
                    }
                    ...
                    if (modConfig) {
                        this.loadScript(modConfig, function () {
                            ...
                            if (modConfig.template) {
                                $http({ cache: $templateCache, url: modConfig.template, method: "get" }).success(function (data) {
                                    //Создаем отдельное пространство для модуля
                                    elm.html(data);
                                    var content = elm.contents(), sc = (options.newScope === undefined) ? scope : scope.$new();
                                    $compile(content)(sc);
                                    afterLoad(content);
                                });
                                //}
    ...
    lazyLoad.directive("lazyLoad", ['$lazyLoad', function ($lazyLoad) {
        return {
